...
MAIN_PROGRAM_INFO = "mensaman v0.5.0"
...
def PrintDateMenu(content, today, options):
    ...
    #      description = description.replace('ä','ae').replace('ö','oe').replace('ü','ue').replace('ß','ss').replace('Ä','Ae').replace('Ö','Oe').replace('Ü','Ue')
          ...
          if (description != "- * - * - *"  and description != "Heute nicht im Angebot!"):
            print ''
            print menus[j],':'
            print description.decode("ISO-8859-1").encode("utf-8")
...
def DisplayHelp(value):
  ...
  print "\n\nHelp manual for", MAIN_PROGRAM_INFO, "\n------------\n"
  ...
  print " --git       display git address"
  print " --version   print version information for mensaman and exits"
  
  print "\n written by Christian Dehn 2014\n\n"
  
  return value

def DisplayVersion(value):
  print MAIN_PROGRAM_INFO;

  sys.exit(0);

  return value

def DisplayGitAddress(value):
  print "\n--------------------------------------------------\n"
  print "		      GIT			   \n"
  print "\nCheck out the all new mensaman git repository!\n"
  print "https://github.com/Aeonos/mensaman.git\n"
  print "--------------------------------------------------\n"
  return value
...
offsets = []
settings = []
allowed_settings = ['--help','--graphics','--extra','--git','--version']
